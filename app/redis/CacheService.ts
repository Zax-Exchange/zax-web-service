...
class CacheService {
  ...
  async setProjectInCache(value: projectsAttributes): Promise<boolean> {
    try {
      ...
      const response: string = (await this.client.SET(cacheKey, valueAsString, {
        PX: CacheService.PROJECT_TTL_MS,
      }))!;
  ...
  async getDetailedProjectInCache(projectId: string): Promise<Project | null> {
    try {
      ...
      console.error(
        `Error retrieving detailed project ${projectId} from cache`,
        err
      );
      return Promise.resolve(null);
    }
  ...
  async getUserInCache(userId: string): Promise<GenericUser | null> {
    try {
      const cacheKey = this.getUserCacheKey(userId);
      const valueInCacheString: string = (await this.client.GET(cacheKey))!;
      const valueInCache = JSON.parse(valueInCacheString) as GenericUser;
      return Promise.resolve(valueInCache);
    } catch (err) {
  ...
  async setUserInCache(value: GenericUser): Promise<boolean> {
    try {
      const userId: string = value.id;
      const cacheKey = this.getUserCacheKey(userId);
      const valueAsString = JSON.stringify(value);
      const response: string = (await this.client.SET(cacheKey, valueAsString, {
        PX: CacheService.USER_TTL_MS,
      }))!;
      return Promise.resolve(response === "OK");
    } catch (err) {
